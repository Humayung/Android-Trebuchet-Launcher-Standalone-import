package com.androidsource.copy;
...
public class TimeAnimator extends Animator {
    ...
    private static final ThreadLocal<ArrayList<TimeAnimator>> sAnimations =
            new ThreadLocal<ArrayList<TimeAnimator>>() {
                @Override
                protected ArrayList<TimeAnimator> initialValue() {
                    return new ArrayList<TimeAnimator>();
                }
            };

    // The per-thread set of animations to be started on the next animation frame
    private static final ThreadLocal<ArrayList<TimeAnimator>> sPendingAnimations =
            new ThreadLocal<ArrayList<TimeAnimator>>() {
                @Override
                protected ArrayList<TimeAnimator> initialValue() {
                    return new ArrayList<TimeAnimator>();
     ...
     */
    private static final ThreadLocal<ArrayList<TimeAnimator>> sDelayedAnims =
            new ThreadLocal<ArrayList<TimeAnimator>>() {
                @Override
                protected ArrayList<TimeAnimator> initialValue() {
                    return new ArrayList<TimeAnimator>();
    ...
    private static final ThreadLocal<ArrayList<TimeAnimator>> sEndingAnims =
            new ThreadLocal<ArrayList<TimeAnimator>>() {
                @Override
                protected ArrayList<TimeAnimator> initialValue() {
                    return new ArrayList<TimeAnimator>();
    ...
    private static final ThreadLocal<ArrayList<TimeAnimator>> sReadyAnims =
            new ThreadLocal<ArrayList<TimeAnimator>>() {
                @Override
                protected ArrayList<TimeAnimator> initialValue() {
                    return new ArrayList<TimeAnimator>();
...
//        }
     ...
     */
    public TimeAnimator setDuration(long duration) {
        if (duration < 0) {
    ...
    private static class AnimationHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            ArrayList<TimeAnimator> animations = sAnimations.get();
            ArrayList<TimeAnimator> delayedAnims = sDelayedAnims.get();
            switch (msg.what) {
                // TODO: should we avoid sending frame message when starting if we
                // were already running?
                case ANIMATION_START:
                    ArrayList<TimeAnimator> pendingAnimations = sPendingAnimations.get();
                    ...
                    while (pendingAnimations.size() > 0) {
                        ArrayList<TimeAnimator> pendingCopy =
                                (ArrayList<TimeAnimator>) pendingAnimations.clone();
                        pendingAnimations.clear();
                        int count = pendingCopy.size();
                        for (int i = 0; i < count; ++i) {
                            TimeAnimator anim = pendingCopy.get(i);
                ...
                case ANIMATION_FRAME:
                    ...
                    // during this frame
                    long currentTime = AnimationUtils.currentAnimationTimeMillis();
                    ArrayList<TimeAnimator> readyAnims = sReadyAnims.get();
                    ArrayList<TimeAnimator> endingAnims = sEndingAnims.get();

                    // First, process animations currently sitting on the delayed queue, adding
                    // them to the active animations if they are ready
                    int numDelayedAnims = delayedAnims.size();
                    for (int i = 0; i < numDelayedAnims; ++i) {
                        TimeAnimator anim = delayedAnims.get(i);
                    ...
                    if (numReadyAnims > 0) {
                        for (int i = 0; i < numReadyAnims; ++i) {
                            TimeAnimator anim = readyAnims.get(i);
                            anim.startAnimation();
                            anim.mRunning = true;
                    ...
                    int i = 0;
                    while (i < numAnims) {
                        TimeAnimator anim = animations.get(i);
    ...
    public Object getAnimatedValue() {
        if (mValues != null && mValues.length > 0) {
        	return getAnimatedValueFromPropertyValuesHolder(mValues[0]);
//            return mValues[0].getAnimatedValue();
    ...
    public Object getAnimatedValue(String propertyName) {
        ...
        if (valuesHolder != null) {
        	return getAnimatedValueFromPropertyValuesHolder(valuesHolder);
//            return valuesHolder.getAnimatedValue();
    ...
    private void startAnimation() {
        ...
        if (mStartDelay > 0 && getListeners() != null) {
...
//        return done;
    ...
    void animateValue(float fraction) {
    	//this should be a no-op in TimeAnimator
...
//        }
    ...
    public TimeAnimator clone() {
        final TimeAnimator anim = (TimeAnimator) super.clone();
        if (mUpdateListeners != null) {
    ...
    public static interface AnimatorUpdateListener {
         ...
         */
        void onAnimationUpdate(TimeAnimator animation);
    ...
    /*
     * reflection on PropertyValuesHolder.java
     * vreified in 4.1, 4.2
     * */
    private Object getAnimatedValueFromPropertyValuesHolder(PropertyValuesHolder holder){
    	if (holder == null) return null;
    	Class<?> classType;
    	Method getAnimatedValueMethod;
    	Object result=null;
    	try {
        	classType = holder.getClass();
			getAnimatedValueMethod = classType.getMethod("getAnimatedValue", new Class[]{});
			getAnimatedValueMethod.setAccessible(true);
			result = getAnimatedValueMethod.invoke(holder, new Object[]{});
		} catch (Exception e) {
			Log.d("trebuchet", "TimeAnimator-getAnimatedValueFromPropertyValuesHolder reflection", e);
		}
    	return result;
    }
