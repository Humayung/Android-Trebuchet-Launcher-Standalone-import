...
import com.androidsource.copy.XmlUtils;
...
public class LauncherProvider extends ContentProvider {
    ...
    private static void deleteId(SQLiteDatabase db, long id) {
        Uri uri = Favorites.getContentUri(id, false);
        SqlArguments args = new SqlArguments(uri, null, null);
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private int copyFromCursor(SQLiteDatabase db, Cursor c) {
            ...
            final int displayModeIndex = c.getColumnIndexOrThrow(Favorites.DISPLAY_MODE);

            ContentValues[] rows = new ContentValues[c.getCount()];
            int i = 0;
            while (c.moveToNext()) {
                ContentValues values = new ContentValues(c.getColumnCount());
                values.put(Favorites._ID, c.getLong(idIndex));
                values.put(Favorites.INTENT, c.getString(intentIndex));
                values.put(Favorites.TITLE, c.getString(titleIndex));
                values.put(Favorites.ICON_TYPE, c.getInt(iconTypeIndex));
                ...
                values.put(Favorites.ITEM_TYPE, c.getInt(itemTypeIndex));
            ...
            try {
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    if (TAG_FAVORITE.equals(name)) {
                        ...
                        String title;
                        int titleResId =  a.getResourceId(R.styleable.Favorite_favorite_title, -1);
                        if (titleResId != -1) {
        ...
        private long addUriShortcut(SQLiteDatabase db, ContentValues values,
            ...
            final int titleResId = a.getResourceId(R.styleable.Favorite_favorite_title, 0);
