...
public class CellLayout extends ViewGroup {
    ...
    private HashMap<LayoutParams, ObjectAnimator> mReorderAnimators = new
            HashMap<LayoutParams, ObjectAnimator>();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!LauncherApplication.isScreenLarge()){
            mCellWidth = mOriginalCellWidth = (widthSpecSize - getPaddingLeft() - getPaddingRight()) / mCountX;
            mCellHeight = mOriginalCellHeight = (heightSpecSize - getPaddingTop() - getPaddingBottom()) / mCountY;
        }
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    //redirect all fast* methods to normal methods
    public void fastInvalidate(){
    	invalidate();
    }
    public void setFastRotationY(float rotationY){
    	setRotationY(rotationY);
    }
    public void setFastScaleX(float scaleX){
    	setScaleX(scaleX);
    }
    public void setFastScaleY(float scaleY){
    	setScaleY(scaleY);
    }
    public void setFastTranslationX(float translationX){
    	setTranslationX(translationX);
    }
    public void setFastTranslationY(float translationY){
    	setTranslationX(translationY);
    }
}
