...
import com.androidsource.copy.Advanceable;
    ...
    protected void onActivityResult(final int requestCode, int resultCode, final Intent data) {
    	if(requestCode == REQUEST_BIND_WIDGET){
    		if (resultCode == RESULT_OK){
    			addAppWidgetImpl(data.getIntExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, 0), pending_add_widget_info);
    		}else{
...
//        ValueAnimator.clearAllAnimations(); //ignore this private method call
    }
    ...
    void addAppWidgetImpl(int appWidgetId, PendingAddWidgetInfo info) {
        ...
        if (appWidget==null){
//        	Toast.makeText(getBaseContext(), "appWidget is null", Toast.LENGTH_SHORT).show();
        	exitSpringLoadedDragModeDelayed(false,false);
    ...
    private static final int REQUEST_BIND_WIDGET = 1987;
    private PendingAddWidgetInfo pending_add_widget_info;
    /**
    ...
    void addAppWidgetFromDrop(PendingAddWidgetInfo info, long container, int screen,
        ...
        boolean bindResult = AppWidgetManager.getInstance(this).bindAppWidgetIdIfAllowed(appWidgetId, info.componentName);
        if (!bindResult){
        	pending_add_widget_info = info;
        	...
        	bindIntent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_PROVIDER, info.componentName);
    ...
    public void onLongClickAppsTab(View v) {
        ...
        popupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                public boolean onMenuItemClick(MenuItem item) {
                    ...
                    if (itemId == R.id.apps_sort_title) {
                        mAppsCustomizeContent.setSortMode(AppsCustomizeView.SortMode.Title);
                    } else if (itemId == R.id.apps_sort_install_date) {
